...
As we already mentioned concurrency bugs are difficult to detect and fix. Many
tools exist to detect concurrency bugs. The identification of a problem is
often not enough for concurrency bugs as multiple code fragments are involved
in the bug, the appropriate fix is not always clear. There has been work in
concurrency anti-patterns that provides a definition, problem, context and
solution of concurrency bugs~\cite{BJ09}.

For example, if we have a \texttt{write} and \texttt{read} methods that
accesses a shared variable. We have a very simple data race bug because there
...
solution to this bug involves two changes, if only one change is present the
bug still exists.  Using this example one of the native solutions to this data
race is to synchronize both methods (as shown in
Figure~\ref{fig:fixed_sample_datarace}):

The presented solution is far from ideal, as it forces other threads to wait
while the write method works on the loop and database sections. An optimization
to the found fix is to shrink the critical region (the statements that are
synchronized) to only guard access to the shared variable (as shown in
...
still presents itself. The developer then synchronizes the \texttt{write}
method as well, and the bug is fixed}
...
loop. The developer shrinks the critical region again to exclude the database
query as a further optimization.}
\label{fig:optimized_sample_datarace}
\end{figure}

From a developer's standpoint there is a lot of work involved in creating a fix
for these types of bugs as multiple changes are not uncommon. As the presented
example illustrated two changes were required to functionally fix the program,
while two additional changes were required to improve the non-functional
performance. Automated tools in software testing are highly desired as it has
the potential to reduce the vast amount of resources spent on software testing
(upwards to \$59.5 billion)~\cite{RTI02}. ARC provides an automated approach to
first fixing the \textit{functionality}, and then optimizing the
