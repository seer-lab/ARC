...
Threads in programs are objects that execute statements. Concurrent programs 
...
Attempts to fix single-threaded and multi-threaded software is examined.
...
for parallel bugs as multiple, unrelated changes are not uncommon. As the example
...
First, the buggy program and second, a JUnit test suite. This
test suite is necessary as it acts like an oracle in determining if the bug
...
The next section details each step in the two phases (as
shown in Figure~\ref{fig:phases_internals}). Differences between them are 
indicated.
...
Thia involves creating a copy of the
...
the non-functional phase ARC has already produced the fixed program, so each 
individual receives a copy of the fixed program.

\subsection{Update Individual's Representation}
\label{sec:update_individual_representation}

An individual is mutated from generation-to-generation based off of its
current representation. This step updates the representation by creating all 
the possible mutant variations for the individual. The next step actually 
applies one of these mutant variants.
...
The set of mutation operators is listed in Table~\ref{tbl:operators}. These
mutation operators are TXL operators, derived from the
ConMAn operators~\cite{BCD06}. TXL operators work by looking for a pattern and
then transforming it according to a set of rules.  An example of one of our
...
mutation operators on the individual's program. This populates
...
example mutation in Figure~\ref{fig:EXSB_example}) creates all the mutants
...
mutations can occur. The search space of possible mutations changes every 
generation. This is why the representation must be updated to account for this.

\subsection{Mutate Individuals}
\label{sec:mutate_individuals}

As suggested in the previous section, the mutation step mutates each 
individual's current program state. The mutation step begins by
...
from the step involved in calculating the operator's weighting. This is further
...
to discover the number of mutation instances from the individual's
representation.  ARC also uses historical information about previous
successes with the mutation operators as well as the current dominating bug type
to select a mutation operator to apply. By considering historical information and bug
type information ARC weighs the selection of mutation operators in favour of  ones
that have been highly succeful in the pas. After the
mutation operator is selected we randomly select an instance to accept as our final
mutation for that individual.

\subsection{Evaluate Individuals}
\label{sec:evalute_individuals}

Each individual is now at a new program state. Now they are evaluated to
determine whether they are in a better state or not. One key problem is the
 unpredictability of thread
...
scheduled interleavings. ConTest is invoked by ARC many times to test each
...
runs. We consider timeouts as they could either result in a deadlock or
...
to ensure it is truly a fix. Found solutions could still hold a
concurrency bug but it was just the situation where none of the error-occurring
thread interleavings were encountered. Our safeguard is to
run ConTest an additional number of times (default is 10x more runs
then the original number of runs). If the fix still holds even after the
safeguard then we feel confident the solution is indeed a true
solution. If any of these additional test fail, the proposed
solution is rejected and the evolutionary process continues.
...
Figure~\ref{fig:nonfunctional_function} shows the non-functional fitness function. 
Our equation will adjust based on the significance and uncertainty of both 
variables (to be fair
...
rejected as it re-introduces a bug. The individual's program is reset to the
previous generation.
...
Each phase continues to evolve and evaluate individuals until some
terminating condition is met. The following conditions are for the
functional phase:
...
stray down a path leading to little or no improvement. To encourage
individuals to explore more of the state space we employ a replacement
strategy that replaces the lower $x$ percentage of individuals with a random
individual from the upper $x$ percent.  These replacements only
occur after an individual has seen at least $y$ new changes. We believe
the original state of the bug is not too far from the solution. To
prevent individuals from straying too far from the original state an individual
is reset to the original state if the individual has not seen any improvement
in $z$ generations.
...
mutation step. A sliding window of $n$ generations is to determine
...
flexibility in changing weighting based on recent history. The weighting is
designed to ensure the chance of selecting an operator is always greater
than zero regardless of performance. To ensure this approach is up to date
...
program is summarized in Table~\ref{tbl:summary_results}. % TODO
...
considering the whole program along with a
large set of mutations. The large space of potential mutations along with the
large set of possible places to apply them causes an extremely large search space for
finding the solution. In ARC we are only considering concurrency bugs and are 
...
The search space can be reduced even
further by considering only the mutation that fall on the error path (used in
Weimer's approach~\cite{GNFW11}). This is future work.

\textbf{Dependence on Test Suite}: ARC depends on a test suite for the 
program being repaired. In both Arcuri's and
Weimer's approach~\cite{AY08,GNFW11} the test suites are used to both guard the
program's functionality and to indicate a bug is present. ARC
requires a test suit to exhibit the bug. ARC currently has no solution for 
dealing with bugs that occur outside of the test suite's coverage. Arcuri's 
and Weimer's approaches also has no solution for this. 

There has been some work that attempts to co-evolve the
test suite at the same time to help check for new situations~\cite{WT10}. We
assume the test suite is comprehensive enough to detect all bugs. ARC will 
eventually only consider using
mutation operators that fall within the coverage of the test suite to avoid
introducing new bugs the test suite cannot detect.

\textbf{Program Readability}: The act of automatically fixing and optimizating
a program raises the question of source readability. None of the related work
considers the readability of their fixes. This is a concern as developers
want to understand the fix as well. For ARC's we are simply adding, removing and
modifying synchronization statements. Posnett et. al.~\cite{PHD11} use the following
...
Readability metrics for programs fixed by ARC are collected in Tabel~\ref{tbl:readability}.
\ldots % TODO
