...
class tester(threading.Thread):
  ...
  def __init__(self, testID, sharedInfo):
    ...
    self.process = subprocess.Popen( ['java', '-cp', 
                      self._sharedInfo.classPath, self._sharedInfo.mainClass,
                      '-javaagent: ' + self._sharedInfo.conTestFile,
                      '-Dcontest.verbose=0'], stdout=subprocess.PIPE, 
                      shell=False)
    self.testID = testID
  ...
  def run(self):
    ...
    if (self.process.poll() == None):
      ...
      # Check to see if there is any deadlock using "Java-level deadlock:"
