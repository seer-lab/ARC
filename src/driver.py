...
class driver():
  ...
  def begin_approach(self):
    ...
    while (not self.is_satisfied(count)):
      ...
      print "~> Number of Deadlocked Runs: " +\
            repr(self._sharedInfo.get_deadlock_count()) +\
            "/" + repr(self._sharedInfo.numOfRuns)
      print "----Completed Generation " + repr(count) + "-----"

    print "\n----------Summary of Genetic Healing Process----------"
    print "~> Number of Generations completed: " + repr(count) + "/" +\
          repr(self._sharedInfo.numOfGenerations)
    print "~> Number of Test Runs in each Generation: " +\
          repr(self._sharedInfo.numOfRuns)
  ...
  def is_satisfied(self, currentGenerations):
    ...
    percentWithNoBugs = 1 - (self._sharedInfo.get_deadlock_count() / 
                        self._sharedInfo.numOfRuns)
